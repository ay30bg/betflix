...
function HistoryTable({ bets, isLoading }) {
  const getNumberColor = (value) => {
    if (value === undefined || value === null) return '';
    return value % 2 === 0 ? 'Green' : 'Red';
  };
  ...
  if (isLoading) {
    return <div className="loading-spinner" aria-live="polite">Loading bet history...</div>;
  }
            ...
            {bets.map((bet, index) => {
              ...
              return (
                <tr key={`${bet.period}-${index}`}>
                  ...
                  <td data-label="Status" className={bet.won === undefined ? 'pending' : bet.won ? 'won' : 'lost'}>
                    {bet.won === undefined ? 'Pending' : bet.won ? 'Won' : 'Lost'}
                  </td>
                </tr>
              );
            })}
