...
function HistoryTable({ bets }) {
  const getNumberColor = (value) => (value % 2 === 0 ? 'Green' : 'Red');

  const getBetDisplay = (bet) => {
    if (bet.type === 'color') {
      return { text: bet.value, color: bet.value };
    }
    return { text: `Number ${bet.value}`, color: getNumberColor(bet.value) };
  };

  const getResultDisplay = (bet) => {
    if (bet.type === 'color') {
      return { text: bet.result, color: bet.result };
    }
    ...
    <div className="history-table-container">
      {bets.length === 0 ? (
        ...
        <table className="history-table" aria-label="Bet History Table">
          ...
          <tbody>
            {bets.map((bet, index) => {
              if (!bet || !bet.type || bet.value === undefined) {
                return (
                  <tr key={index}>
              ...
              return (
                <tr key={`${bet.period}-${index}`}>
                  ...
                  <td data-label="Bet" className={`color-${betDisplay.color.toLowerCase()}`}>
                    {betDisplay.text}
                  </td>
                  <td data-label="Amount">${bet.amount.toFixed(2)}</td>
                  <td data-label="Result" className={`color-${resultDisplay.color.toLowerCase()}`}>
                    {resultDisplay.text}
                  </td>
