...
import Header from '../components/header';
import '../styles/profile.css';
import { FaUser, FaBitcoin, FaCopy, FaMoneyBillWave } from 'react-icons/fa';
import { QRCodeCanvas } from 'qrcode.react';
...
function Profile() {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const { balance, setBalance, isLoading: balanceLoading, error: balanceError, addFakeMoney } = useBalance();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isDepositModalOpen, setIsDepositModalOpen] = useState(false);
  const [isWithdrawModalOpen, setIsWithdrawModalOpen] = useState(false);
  const [isFakeMoneyModalOpen, setIsFakeMoneyModalOpen] = useState(false); // New state for fake money modal
  const [depositResult, setDepositResult] = useState(null);
  const [formData, setFormData] = useState({ username: '' });
  const [depositData, setDepositData] = useState({ amount: '', cryptoCurrency: 'BTC', network: 'ERC20' });
  const [withdrawData, setWithdrawData] = useState({ amount: '', cryptoCurrency: 'BTC', walletAddress: '', network: 'ERC20' });
  const [fakeMoneyData, setFakeMoneyData] = useState({ amount: '' }); // State for fake money input
  const [errors, setErrors] = useState({ profile: '', deposit: '', withdraw: '', fakeMoney: '' }); // Add fakeMoney error
  const [notification, setNotification] = useState(null);
  ...
  // Handler for fake money submission
  const handleAddFakeMoney = (e) => {
    e.preventDefault();
    const amount = parseFloat(fakeMoneyData.amount);
    if (isNaN(amount) || amount <= 0) {
      setErrors((prev) => ({ ...prev, fakeMoney: 'Please enter a valid amount greater than 0' }));
      return;
    }
    addFakeMoney(amount);
    setNotification({ type: 'success', message: `Added $${amount.toFixed(2)} fake money to balance!` });
    setIsFakeMoneyModalOpen(false);
    setFakeMoneyData({ amount: '' });
    setErrors((prev) => ({ ...prev, fakeMoney: '' }));
  };
          ...
          <div className="profile-button-group">
            ...
            {/* Add Fake Money Button (Development Only) */}
              ...
              <button
                ...
                aria-label="Add fake money for testing"
              >
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            <button
              ...
              className="modal-close"
              aria-label="Close fake money modal"
            >
              ×
            </button>
            <h2>Add Fake Money</h2>
            <form onSubmit={handleAddFakeMoney}>
              <div className="form-group">
                <label htmlFor="fake-money-amount" className="modal-label">
                  Amount (USD)
                </label>
                <input
                  id="fake-money-amount"
                  type="number"
                  step="0.01"
                  min="0.01"
                  value={fakeMoneyData.amount}
                  onChange={(e) =>
                    setFakeMoneyData({ ...fakeMoneyData, amount: e.target.value })
                  }
                  className="modal-input"
                  placeholder="Enter amount in USD"
                />
              </div>
              {errors.fakeMoney && <p className="modal-error">{errors.fakeMoney}</p>}
