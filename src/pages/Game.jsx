...
function Game() {
  ...
  const { data: betsData, isLoading: betsLoading, error: betsError } = useQuery({
    ...
    retry: (failureCount, error) => failureCount < 3 && !error.message.includes('Authentication'),
  });
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setPendingBet(data.bet);
      localStorage.setItem('pendingBet', JSON.stringify(data.bet));
      setError('');
    },
    onError: (err) => {
      const errorMessage = err.message.includes('Authentication required')
    ...
    return (
      <ErrorBoundary>
        <div className="game-page container">
          <Header />
          <p className="game-error" role="alert" aria-live="polite">
            Failed to load bets: {betsError.message}
          </p>
          <button
            onClick={() => queryClient.invalidateQueries(['bets'])}
            aria-label="Retry fetching bets"
          >
            Retry
          </button>
        </div>
