...
//   }
//   const bets = await response.json();
//   // Only filter out bets with status 'invalid'
    ...
    onSuccess: (data) => {
      ...
      data.forEach((bet) => {
        if (bet.status === 'pending' && bet.roundExpiresAt && new Date(bet.roundExpiresAt) < new Date()) {
          ...
          setNotification({
            type: 'warning',
            message: `Bet for round ${bet.period} could not be settled: ${bet.error || 'Unknown error'}. Please refresh.`,
          });
          if (bet === pendingBet) {
            setPendingBet(null);
          }
