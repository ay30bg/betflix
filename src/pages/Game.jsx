...
function Game() {
  ...
  const handleAuthError = useCallback((message) => {
    setNotification({ type: 'error', message: 'Session expired. Please log in again.' });
    localStorage.removeItem('token');
    localStorage.removeItem('pendingBet'); // Clear pending bet on auth error
    setTimeout(() => {
      navigate('/login');
    }, 3000);
  }, [navigate]);

  // Restore pending bet from localStorage on mount
  useEffect(() => {
    ...
    if (storedBet) {
      try {
        ...
        console.error('Failed to parse stored pending bet:', err);
        localStorage.removeItem('pendingBet');
      }
    }
  }, []);

  // Force refetch bets on mount
  useEffect(() => {
    queryClient.invalidateQueries(['bets']);
  }, [queryClient]);
  ...
  // Check betsData for pending bets and set pendingBet
  useEffect(() => {
    if (betsData && !pendingBet) {
      const latestPendingBet = betsData.find(
        (bet) => bet.won === undefined && bet.result === undefined && /^round-\d+$/.test(bet.period)
      );
      if (latestPendingBet) {
        setPendingBet(latestPendingBet);
        localStorage.setItem('pendingBet', JSON.stringify(latestPendingBet));
      }
    }
  }, [betsData, pendingBet]);
  ...
  const fetchResult = useCallback(
    async (retryCount = 3) => {
      ...
      try {
        ...
        const data = await fetchBetResult(pendingBet.period);
        if (!data.bet) {
          console.warn('No bet data returned');
          setError('Bet not found for this round');
          setPendingBet(null);
          localStorage.removeItem('pendingBet');
          return;
        }
        if (typeof data.bet.result === 'undefined' || typeof data.bet.won === 'undefined') {
          console.warn(`Result not ready, retries left: ${retryCount}`);
          if (retryCount > 0) {
            setTimeout(() => fetchResult(retryCount - 1), 3000);
            return;
        ...
        if (
          ...
          err.message.includes('Round has expired') ||
          err.message.includes('Bet not found') ||
          err.message.includes('Invalid period')
        ) {
          setPendingBet(null);
          localStorage.removeItem('pendingBet');
        }
  ...
  // Trigger fetchResult at 15 seconds or if pendingBet is restored
  useEffect(() => {
  ...
  }, [pendingBet, timeLeft, fetchResult, lastResult]);
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setPendingBet(data.bet);
      localStorage.setItem('pendingBet', JSON.stringify(data.bet)); // Persist pending bet
      setError('');
    },
    onError: (err) => {
      const errorMessage = err.message.includes('Authentication required')
