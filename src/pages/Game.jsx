...
//           isLoading={mutation.isLoading}
//           balance={balance ?? 0}
//           isDisabled={(balance ?? 0) === 0 || mutation.isLoading || timeLeft < 15 || !!pendingBet}
//           roundData={roundData}
//           timeLeft={timeLeft}
//         />
//         <HistoryTable bets={betsData || []} />
...
function Game() {
  ...
  const { data: pendingBetsData, isLoading: pendingBetsLoading } = useQuery({
    ...
    onSuccess: (data) => {
      const activeBets = data.filter(
        (bet) => bet.status === 'pending' && bet.roundStatus === 'active'
      );
      if (activeBets.length > 0 && !pendingBet) {
        setPendingBet(activeBets[0]);
        setNotification({
          type: 'info',
          message: `Restored pending bet for round ${activeBets[0].period}`,
        });
      }
      data.forEach((bet) => {
        if (bet.status === 'pending' && bet.roundExpiresAt && new Date(bet.roundExpiresAt) < new Date()) {
  ...
  const handleBet = async (betData) => {
    ...
    try {
      ...
      const errorMessage = err.message.includes('Authentication required')
        ? 'Session expired. Please log in again.'
        : err.message.includes('Maximum of 2 bets allowed')
        ? 'You have already placed the maximum of 2 bets for this round.'
        : err.message;
      setError(errorMessage);
      setTimeout(() => setError(''), 5000);
      if (err.message.includes('Authentication required')) {
  ...
  // Count pending bets for the current round
  const currentRoundPendingBets = pendingBetsData?.filter(
    (bet) => bet.status === 'pending' && bet.period === roundData?.period
  )?.length || 0;
      ...
      <div className="game-page container">
        ...
        {pendingBet && !lastResult && !mutation.isLoading && (
          <div className="no-result" role="alert" aria-live="polite">
            <p>Bet placed on {pendingBet.period}.</p>
            <p>Waiting for results... ⌛</p>
            {currentRoundPendingBets === 1 && (
              <p>You can place one more bet for this round.</p>
            )}
          </div>
        )}
        {!pendingBet && !lastResult && !mutation.isLoading && (
