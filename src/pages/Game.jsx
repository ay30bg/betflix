  ...
  useEffect(() => {
    const handleBeforeUnload = (event) => {
      if (pendingBet && timeLeft > 0) {
        ...
        event.returnValue = 'A bet is active for this round. Leaving may cause issues. Are you sure?';
      }
    };
  ...
  const fetchResult = useCallback(
    async (retryCount = 3) => {
      ...
      try {
        ...
        await queryClient.invalidateQueries(['bets']); // Ensure bets are refreshed
        queryClient.invalidateQueries(['stats']); // Invalidate stats to refresh Profile stats
        setPendingBet(null);
      } catch (err) {
        const errorMessage = err.message.includes('Authentication required')
