...
function Game() {
    ...
    async (period, retryCount = 3) => {
      ...
      try {
        ...
        const errorMessage = err.message.includes('Authentication required')
          ...
          : err.message.includes('Bet not found')
          ? 'No bet found for this round'
          : err.message.includes('Round has not yet expired')
  ...
  useEffect(() => {
    if (pendingBet && roundData?.expiresAt && new Date(roundData.expiresAt) <= new Date() && !lastResult) {
      debouncedFetchResult(pendingBet.period);
    }
  }, [pendingBet, roundData, debouncedFetchResult, lastResult]);
