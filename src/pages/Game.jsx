...
const fetchRecentRounds = async () => {
  ...
  console.log('Fetched recent rounds:', rounds);
  return rounds;
};
...
function Game() {
  ...
  // Fetch profile on mount to ensure latest balance
  useQuery({
    queryKey: ['profile'],
    queryFn: fetchProfile,
    onSuccess: (data) => {
  ...
  const { data: recentRoundsData, isLoading: recentRoundsLoading } = useQuery({
  ...
  // Render loading state
  if (balanceLoading || betsLoading || roundLoading || recentRoundsLoading || pendingBetsLoading) {
    return (
      <ErrorBoundary>
        <div className="game-page container">
          <Header />
      ...
      <div className="game-page container">
        ...
        <div className="game958">
          <div className="round-info">
            ...
            <button
              className="history-button"
              onClick={() => setIsHistoryModalOpen(true)}
              aria-label="View last 10 rounds history"
            >
              View History
            </button>
          </div>
        ...
        {isHistoryModalOpen && (
          <div className="modal-overlay" role="dialog" aria-labelledby="history-modal-title">
            <div className="modal-content">
              <div className="modal-header">
                <h2 id="history-modal-title">Recent Rounds History (Last 10)</h2>
                <button
                  className="modal-close"
                  onClick={() => setIsHistoryModalOpen(false)}
                  aria-label="Close modal"
              ...
              <div className="modal-body">
                {recentRoundsLoading ? (
                  <div className="loading-spinner" aria-live="polite">Loading recent rounds...</div>
                ) : (
                  <table className="history-table">
                    <thead>
                      <tr>
                    ...
                    <tbody>
                      {recentRoundsData?.length > 0 ? (
                        ...
                        <tr>
                          <td colSpan="3">No past rounds available</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
